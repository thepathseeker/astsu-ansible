...
This repository uses Ansible version that requires Python 3.12.
...
```bash
TMPDIR="${HOME}/tmp" python3.12 -m pip install --user ansible==13.0.0 passlib
```
...
```bash
python3.12 -m pip install --user ansible-lint==25.11.0
```
...
```bash
python3.12 -m pip install --user pre-commit==4.5.0
```
...
# the password is "security-breach" also stored in file vault.key
```
